  ...
  public Maze maze(){
   ...
   /* public void handleMove(Point2D start){
        ...
        if(wallB!=null) {
            ...
            Vector3D wallDirection=new Vector3D(wallLine.getP2()).subtract(new Vector3D(wallLine.getP1())).normalize();
            Vector3D toCenter=new Vector3D(goal).subtract(new Vector3D(wallLine.getP1()));
            Vector3D perpComponent=toCenter.subtract(wallDirection.multiply(toCenter.dotProduct(wallDirection)));
            double penetrationDepth=radius-perpComponent.norm();

            Vector3D correction=perpComponent.normalize().multiply(penetrationDepth);
            goal.setLocation(goal.getX()+correction.x(),goal.getY()+correction.z());

            Vector3D mvt = new Vector3D(collide).subtract(new Vector3D(goal));
            Vector3D wallNormal = wallNormal(collide, wallLine);
            System.out.println(wallNormal.x()+"   "+wallNormal.y()+"   "+wallNormal.z());
